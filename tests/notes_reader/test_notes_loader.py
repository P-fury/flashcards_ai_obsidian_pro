...
def test_find_tags_multiline_note(note_docker):
...
def test_check_tags_from_note_without_matching_tags():
    nl = MarkdownNotesLoader('.', ['python', '#pytest'])
    assert not nl.check_tags({'#unit_test', '#docker'})
...
def test_get_file_list_returns_no_files(monkeypatch, notes_loader):
    def stub_listdir(*args):
        return []
...
def test_get_file_list_returns_no_files_with_md_extensions(monkeypatch, notes_loader):
    def stub_listdir(*args):
